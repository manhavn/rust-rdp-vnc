...
// Modified for rust-rdp-vnc: complete GNOME connect-time auto-detection and
// advertise the negotiated Dynamic Virtual Channel Graphics protocol.
...
pub struct ClientConnector {
    ...
    pub message_channel_id: Option<u16>,
    connect_time_bandwidth: Option<(Instant, u32)>,
}
                ...
                if let Some((message_channel_id, data)) = message_channel_pdu {
                    if let Ok(autodetect) = decode::<rdp::autodetect::AutoDetectReqPdu>(&data.user_data) {
                        let written = respond_to_connect_time_autodetect(
                            ...
                            message_channel_id,
                            user_channel_id,
                            &mut self.connect_time_bandwidth,
                            output,
                        )?;
                        (
                            written,
...
fn respond_to_connect_time_autodetect(
    ...
    let response = match request {
        ...
        AutoDetectRequest::BandwidthMeasureStart { .. } => {
            *bandwidth_measurement = Some((Instant::now(), 0));
            None
        }
        AutoDetectRequest::BandwidthMeasurePayload { payload, .. } => {
            if let Some((_, byte_count)) = bandwidth_measurement {
                *byte_count = byte_count.saturating_add(u32::try_from(payload.len()).unwrap_or(u32::MAX));
            }
            None
        }
        AutoDetectRequest::BandwidthMeasureStop {
            ...
            let (started_at, mut byte_count) =
                bandwidth_measurement.take().unwrap_or_else(|| (Instant::now(), 0));
            if let Some(payload) = payload {
                byte_count = byte_count.saturating_add(u32::try_from(payload.len()).unwrap_or(u32::MAX));
            }
            let time_delta_ms =
                u32::try_from(started_at.elapsed().as_millis()).unwrap_or(u32::MAX).max(1);
